...
class GiftcodeCreatedEmail extends Mailable implements SettingableMail
{
    ...
    public function build()
    {
        ...
        $setting['body'] = str_replace('{{registration_fee_included_or_not}}',(empty($this->giftcode->registration_fee_in_usd) OR is_null($this->giftcode->package_name)) ? 'No': 'Yes',$setting['body']);
        $setting['body'] = str_replace('{{total_cost}}',is_null($this->giftcode->total_cost_in_usd) ? 'Unknown': (float) $this->giftcode->total_cost_in_usd,$setting['body']);
