...
class WithdrawProfitRequestedSubmitted extends Mailable implements SettingableMail
{
    ...
    public function build()
    {
        ...
        $setting['body'] = str_replace('{{amount_in_pf}}',(is_null(abs($this->withdrawRequest->withdrawTransaction->amountFloat)) ) ? 'Unknown': walletPfAmount((int)abs($this->withdrawRequest->withdrawTransaction->amountFloat)),$setting['body']);
        $setting['body'] = str_replace('{{amount_in_btc}}',(is_null($this->withdrawRequest->withdrawTransaction->amountFloat) ) ? 'Unknown': walletConvertAmountToDecimal((int)abs($this->withdrawRequest->withdrawTransaction->amountFloat)),$setting['body']);
